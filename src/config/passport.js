...
const setUser = (profile, user, provider, done, err) => {
  user = new User();
  user.newUser = true;
  ...
  passport.use(new TwitterStrategy(
    ...
    ((token, tokenSecret, profile, done) => {
      User.findOne({
        ...
        if (!user) {
          setUser(profile, user, 'twitter', done, err);
        } else {
          user.profileId = profile.id;
  ...
  passport.use(new FacebookStrategy(
    ...
    ((accessToken, refreshToken, profile, done) => {
      User.findOne({
        ...
        if (!user) {
          setUser(profile, user, 'facebook', done, err);
        } else {
          user.facebook = null;
          return done(err, user);
        }
  ...
  passport.use(new GitHubStrategy(
    ...
    ((accessToken, refreshToken, profile, done) => {
      ...
      User.findOne({
        ...
        if (!user) {
          setUser(profile, user, 'github', done, err);
        } else {
          return done(err, user);
        }
      });
  ...
  passport.use(new GoogleStrategy(
    ...
    ((accessToken, refreshToken, profile, done) => {
      User.findOne({
        ...
        if (!user) {
          setUser(profile, user, 'google', done, err);
        } else {
          return done(err, user);
        }
      });
