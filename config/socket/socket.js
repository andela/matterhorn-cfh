...
module.exports = function (io) {
  ...
  var getGame = function (player, socket, requestedGameId, createPrivate) {
    ...
    if (requestedGameId.length && allGames[requestedGameId]) {
      ...
      if ((game.state === 'awaiting players' && (game.players.length < game.playerMaxLimit)) && (!game.players.length ||
        ...
        if (game.players.length >= game.playerMaxLimit) {
          ...
          game.state = 'Players Complete';
          game.prepareGame();
        }
      } else {
        game.sendNotification('MAX PLAYERS IN GROUP CURRENTLY!!!');
